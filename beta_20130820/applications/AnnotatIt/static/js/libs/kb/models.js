(function($) {
     ...
     * @type {Backbone.Model}
     */
    AnnotatIt.Note = Backbone.Model.extend({
        //url: function() {return '/api/fscomment/'+this.id+'.json' },
        urlRoot:  '/api/note',
        idAttribute: 'id',
        user : new AnnotatIt.User(),
        ...
        initialize: function(){
            ...
            this.user.id = this.get("owner_id");
            this.user.fetch();
            if (this.get("begin_time") - this.get("end_time") < 0.0001 ){
                this.isSection = true;
            }else{
     ...
     * @contains Comment
     */
    AnnotatIt.NotesContainer = Backbone.Collection.extend({
        self : this,
	    model: AnnotatIt.Note,
        url: function() {return '/api/notes/'+this.media_id+'.json' },
        media_id : -1,
        //idAttribute: 'id',
	    defaults: {
		    text: "Please write your comment here",
     ...
     * @type {Backbone.Model}
     */
    AnnotatIt.Script = Backbone.Model.extend({
        //url: function() {return '/api/fscomment/'+this.id+'.json' },
        urlRoot: function() {
                    if (this.attributes.action && this.attributes.key){
                        return '/api/media_script/'+this.attributes.media_id+'.json' 
                    }else{
                        return '/api/script'
                    }
                 },
     ...
     * @contains Comment
     */
    AnnotatIt.ScriptsContainer = Backbone.Collection.extend({
        self : this,
	    model: AnnotatIt.Note,
        url: function() {return '/api/notes/'+this.media_id+'.json' },
        media_id : -1,
