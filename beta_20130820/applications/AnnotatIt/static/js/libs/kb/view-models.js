(function($) {
    ...
    */
    ...
    AnnotatIt.ScriptVM = kb.ViewModel.extend({
        constructor: function(model) {
            var self = this;
            kb.ViewModel.prototype.constructor.call(this, model);
            self.scriptSelected = ko.observable(false);
            self.Edit = function() {
                self.editing(true);
            };
            self.Save = function() {
                $.event.trigger("Annotatit:Event:Script:update", self.model );//TODO change event type
                self.editing(false);
...
////////////////////////////////////////////////////////////////////////////////
    /**
     * NoteVM: ViewModel for note
     * @type {kb.ViewModel}
     * @attribute comment
     * @param  {Note} model
     * @return {NoteVM}   
     */
    AnnotatIt.NoteVM = kb.ViewModel.extend({
        constructor: function(model) {
            var self = this;
            kb.ViewModel.prototype.constructor.call(this, model);
            //related user's model. Needs KnockBack observables
            ...
            self.toggleHighlight = function(){ 
                //console.log("toggling hightlight");
                self.highlighted(!self.highlighted());
            }
            
            //BEGIN edition fields (for the user interface not to reorder notes 
            // UNTIL the changes are applied by the user (if moves during it is anoying)
            self.new_begin_time = ko.observable(self.begin_time());
            self.new_end_time = ko.observable(self.end_time());
            self.newText = ko.observable(self.text());
            self.newBeginTimetxt = ko.computed( function() { return number2txttime(self.new_begin_time()) } );
            self.newEndTimetxt = ko.computed( function() { return number2txttime(self.new_end_time()) } );
            //END editing fields
            ...
            //methods
            $(document).on("Annotatit:Event:MediaPlayerFacade:time:update", function(event, time) {
                //Take in account clicks on the timeline when editingn note
                if(self.editing() && self.begin_time() == self.end_time()){
            ...
            self.seekEndTime = function(){
                //console.log("seekEndTime");
                //console.log("seek Time: "+ self.end_time() );
                //emit time signal
                //MUST be a number
                $.event.trigger("Annotatit:Event:time:set", self.end_time());
            };

            self.incrementBeginTime = function(){
                //console.log("increment begin time");
                if (self.begin_time() == self.end_time()){
                    //console.log("it is a note");
                    self.new_begin_time(self.new_begin_time() +1);
                    self.new_end_time(self.new_begin_time());
                    //console.log("changed time to");
                    //console.log(self.begin_time());
                }else{
                    self.begin_time(self.new_begin_time() +1);
                    if (self.new_begin_time() > self.new_end_time()){
                        self.new_begin_time(self.new_end_time() -1 );
                    }
                }
                //TODO control that is not bigger than duration of the file
            };
            ...
            self.decrementBeginTime = function(){
                console.log("decrement begin time");
                if (self.begin_time() == self.end_time()){
                    self.new_begin_time(self.new_begin_time() -1);
                    self.new_end_time(self.new_begin_time());
                }else{
                    self.new_begin_time(self.new_begin_time() -1);
                    if (self.new_begin_time() < 0){
                        self.new_begin_time(0);
                    }
                }
            };
            
            self.incrementEndTime = function(){
                self.new_end_time(self.new_end_time() +1);
                //TODO control that is not bigger than duration of the file
            };
            ...
            self.decrementEndTime = function(){
                self.new_end_time(self.new_end_time() -1);
                if (self.new_begin_time() > self.new_end_time()){
                    self.new_end_time(self.new_begin_time() +1 );
                }
                if (self.new_end_time() < 0){
                    self.new_end_time(0);
                }
            };
            self.Edit = function() {
            ...
            self.cancelEditing = function(){
                self.new_begin_time(self.begin_time());
                self.new_end_time(self.end_time());
                self.newText(self.text());
            };
            self.Save = function() {
                //console.log("saving note model");
                //modify the values in the original
                //console.log(self.model());
                self.begin_time(self.new_begin_time());
                self.end_time(self.new_end_time());
                ...
                self.editing(false);
                var ret = self.model().save();
                $.event.trigger("Annotatit:Event:Note:update", self.model );//TODO change event type
                return ret;
            };
            self.Destroy = function() {
                //console.log("deleting note model");
                var ret = self.model().destroy();
                $.event.trigger("Annotatit:Event:Note:update", self.model );//TODO change event type
                return ret;
            }
    ...
    /**
     * NotesListVM: ViewModel for Notes
     * @attribute notes
     * @param  {Notes} Notes 
     * @return {NotesListVM}
     */
    AnnotatIt.NotesListVM = function(models) {
        var self = this;
        // Data
        self.new_text = ko.observable("");
        self.new_begin_time = ko.observable(0); 
        ...
        self.notes = kb.collectionObservable(models, {
            view_model: AnnotatIt.NoteVM
        });
        self.notes.sortAttribute('begin_time');
        self.notesFormSelected = ko.observable(false);
        self.notes.sort();
        ...
        self.allFlags = ko.observableArray(flagList);
        self.selectedFlag = ko.observable(flagList[8]);
        self.flagColor = ko.computed(function(){
            console.log()
            return self.selectedFlag().flag_color;
        });
        
        //FILTERS and highlighting
        self.highlightList = []
        $(document).on("Annotatit:Event:Plot:hoverList", function(event, hoverList) {
            //TODO highlight the selected notes in the graph
            //console.log("hoverList");
            //console.log(hoverList);
            //Get the list of ids
            var idsList = _.map(hoverList, function(m){ return m.get("id"); } );
            //console.log("ids list");
            //console.log(idsList);
            var highlight = self.notes();
            //filter by user
            filtered = ko.utils.arrayFilter(highlight, function(c){
                //console.log("filtering");
                //console.log(c.id());
                return _.contains(idsList, c.id());
            });
            //console.log(filtered);
            //TODO unhighlight the old highlight list
            _.each(self.highlightList, function(d){d.toggleHighlight()});
            
            //TODO now get the notes to highlight!!
            //set the history (keep track of highlighted things)
            self.highlightList = filtered;
            _.each(self.highlightList, function(d){d.toggleHighlight()});
        });
        //$(document).on("Annotatit:Event:Plot:currentTimeList", function(event, currentTimeList) {
        //    //TODO highlight the current time (and scroll if necessary)
        //    //console.log("currentTimeList");
        //});
        ///
        ...
        self.allAuthors = ko.computed(function(){
            ...
            var tmp = _.map(self.notes(), function(s){
                                                //console.log(s);
                                                //console.log(s.first_name);
                                                var ret2 =  s.first_name();
                                                if(ret2){
        ...
        self.filteredNotes = ko.computed(function(){
            ...
            if ( $.trim(self.contentFilter()) !=""){
                filtered = ko.utils.arrayFilter(filtered, function(c){
                    return ( c.text().match(self.contentFilter()));//TODO improve performance, this might be too heavy for too many notes
                });
            }
            //console.log(filtered);
            return filtered;
        });
        //Keeping track of the state of note creation
        self.inprogress = ko.observable(false);
        self.notinprogress = ko.computed( function(){ return !self.inprogress() } );
        self.makeSection = ko.observable(false); //signals that the user wants a section and not a single time point note)

        //this takes in account if begin time should be updated as handles 
        // notes on start writing AND empty notes in section mode
        self.shouldStall = ko.computed(function(){
           return self.inprogress() || self.makeSection(); 
        });
        self.selectFlag = function(flag_name, flag_color, letter){
            //console.log("select new color");
            //console.log(flag_name);
            ...
            //console.log(letter);
            //I know it is not efficient. dont worry be huggies.
            self.selectedFlag(new CommentFlag(flag_name, flag_color, letter));
            //now focus the box and stop the time update!! (a new comment should be created)
            self.notesFormSelected(true);
            //change background color!!
        };
        
        self.notesSelected = ko.observable(true);

        self.selectNotes = function(){
            self.notesSelected(true);
            self.notesFormSelected(true);
        };
        // Update time (event binding)
        $(document).on("Annotatit:Event:MediaPlayerFacade:time:update", function(event, time) {   
            ...
            if(self.makeSection()){ //marks the user's will to create a section and not a simple comment
                self.new_end_time(time);
            }
        });
        // Update time (event binding)
        $(document).on("Annotatit:Event:time:set", function(event, time) {
            console.log("time SET");
            self.inprogress(false);
            self.makeSection(false);
            //self.updateBeginTime = true;
            //self.updateEndTime = true;
        });
        //creation functions
        self.noteStart = function() {
            console.log("Note Start call");
            //self.updateBeginTime = false;
            self.inprogress(true);
            self.makeSection(true);
        };
        self.noteEnd = function() {
            console.log("Note End call");
            self.addNote();
            self.inprogress(false);
            self.makeSection(false);
            self.new_text("");
        };
        self.noteCancel = function() {
            self.new_text("");
            self.inprogress(false);
            self.makeSection(false);
        };
        //creation functions
        self.createNote = function(view_model, event) {
            //console.log("key up");
            //console.log(event.keyCode);
            //if ($.trim(self.new_text()) && (event.keyCode == ENTER_KEY)) {
            if ( event.keyCode == ENTER_KEY ) {
        ...
        self.addNote = function() {
            ...
            //taking timeOffset in account
            if(self.new_begin_time() == self.new_end_time()){
                //case when a note is a single time point comment
                self.new_end_time(self.new_end_time()+self.timeOffset());
            }//else it is a section and end_time is set accordingly
            //begin time always will take in account the timeOffset
            self.new_begin_time(self.new_begin_time()+self.timeOffset())
            //console.log("");
            var note = self.notes.collection().create({
                owner_id: AnnotatIt.me.id,
                media_id: AnnotatIt.media_file.id, //current media id, gets the media id from GLOBAL var
                text: self.new_text(),
                begin_time: self.new_begin_time(),
            ...
            //console.log(note);
            $.event.trigger("Annotatit:Event:Note:update", note );
            return note;
        };
