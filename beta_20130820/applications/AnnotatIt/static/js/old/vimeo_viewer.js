...
function canvas_draw_tag( tag_json ){
    ...
    var xpos =  (ntime/video_duration ) * 720;
    var ypos = 0;
...
function on_tag_click(canvas, obj, tag_json){
    ...
    vmplayer.play();
    //TODO play video api call vmplayer.playVideo();
}
...
function init_vimeo_player(){
    ...
    var obj = vmplayer.api('getDuration', function (value, player_id) {
            //HACK with lauch event because I don't kknow WHY it does not modify the globals or locals outside this function (I suppose is the permission crosscripting problem with the API)
            console.log("val = "+value);
            var evt = document.createEvent("CustomEvent");
            evt.initCustomEvent("customApiEvent", true, true, data={ fcall:'getDuration', val:value});
            console.log(evt);
...
function custom_event_handler(e){
    //tool for managing custom events, last resort to get data from the vimeo  API
    console.log("custom event "+ e);
    if( e.detail.fcall.trim() == 'getDuration'){
            $("#video_duration").text(e.detail.val);
            $("#video_duration").val(e.detail.val);
            video_duration = e.detail.val;
